            ...
            @if(isset($pages) && is_array($pages))
                @foreach($pages as $id => $page)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            {{ date('M d, Y', $page['created_at']) }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('pages.edit', $id) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                            <form action="{{ route('pages.destroy', $id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
