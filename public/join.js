document.addEventListener("DOMContentLoaded", function () {
    ...
    // Create joinable forums container if it doesn't exist
    let joinableForumsContainer = document.getElementById('joinable-forums');
    if (!joinableForumsContainer) {
        joinableForumsContainer = document.createElement('div');
        joinableForumsContainer.id = 'joinable-forums';
        
        // Append it to the forumContainer if that exists
        if (forumContainer) {
            forumContainer.appendChild(joinableForumsContainer);
        } else {
            // If forumContainer doesn't exist, append to the body
            document.body.appendChild(joinableForumsContainer);
            console.log('Warning: .ursac-content-main not found, appending to body instead');
        }
    }
    ...
    forumRef.once('value', (snapshot) => {
        ...
        if (forums) {
            Object.values(forums).forEach(forum => {
                ...
                // Safely append to joinable forums container
                joinableForumsContainer.appendChild(card);

                // Prevent joining own forum
                ...
                if (forum.requiresApproval === false) {
                    ...
                    joinButton.addEventListener('click', () => {
                        ...
                        if (currentUser && forum.id) {
                            const forumRef = firebase.database().ref(`forums/${forum.id}/members/${currentUser.uid}`);
                            forumRef.set({
                                ...
                                return forumJoinedNotifRef.set({
                                    ...
                                    message: `joined the forum: ${forum.name || 'Unnamed Forum'}`,
                                    read: false,
            ...
            // Display message if no forums are available to join
            if (noForumsAvailable && forumContainer) {
                forumContainer.innerHTML += `<p style="color:white;">No Forums Available to Join</p>`;
            }
    ...
    // Set up search functionality only if the search input exists
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function () {
            ...
            if (noResultsMessage) {
            ...
            } else if (visibleCount === 0 && forumContainer) {
                ...
                forumContainer.appendChild(noResults);
            } else {
